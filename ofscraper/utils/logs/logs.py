...
def print_latest_version():
    ...
    with  BareSession.get_bare_session(url="https://pypi.org/pypi/ofscraper/json",method="get") as sess:
        ...
        if re.search(new_version, __version__):
            log.error("[bold yellow]Application is up to date[/bold yellow]")
        elif __version__ == "0.0.0":
            log.error(
                "[bold yellow]Version check unavailable (standalone installation)[/bold yellow]"
            )
        elif ".dev" in __version__:
            log.error("[bold yellow]Development version in use[/bold yellow]")
        else:
            log.error(
                f"[bold yellow]New version available[/bold yellow]: [bold]{new_version}[/bold]"
            )
